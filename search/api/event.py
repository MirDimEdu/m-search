...
def get_storage() -> storage.Storage:
    return storage.Storage(db.get_even_collection())
...
async def put(item: models.SearchItem):
    try:
        ...
        print(f'Failed to put event {item.id}: {repr(e)}')
    return models.StatusResponse(
        status='ERROR'
    )
...
async def delete(id: int):
    try:
        ...
        print(f'Failed to delete event {id}: {repr(e)}')
    return models.StatusResponse(
        status='ERROR'
    )
